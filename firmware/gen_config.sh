...
for BOARD in \
   ...
   "config/boards/hellen/hellen128 hellen128 gerefi_hellen128mercedes.ini" \
...
# reality: while we have per-target signature*.h and gerefi*.ini we do not have matching per-target 'engine_configuration_generated_structures.h'
...
# firmware/controllers/generated/gerefi_generated.h file which would be pushed into VCS
