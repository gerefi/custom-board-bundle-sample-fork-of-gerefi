...
### BIG OPEN QUESTION: do we need to merge this custom BL into OpenBLT? See https://github.com/gerefi/gerefi/wiki/Firmware-update-via-CAN

*** User Manual ***

To start the bootloader updater:
- Turn off your gerefi ECU board, and connect UART to PC.
...
- select the firmware file (gerefi.hex). Note! ***Use only recent firmware builds with bootloader support!***
...
- The compiled bootloader code is stored in bootloader/bootloader_generated.hxx and it can be included into the main firmware (build/gerefi.hex) if the bootloader support is enabled.
